...
- 之前那段是自己拼的请求，一次瞬时抖动（实测 `unexpected EOF`）就让规格缓存落空，
  表现为"外部同名模型没被补上规格"。现在改走与额度请求相同的 `clineRequest`：
  传输层抖动与 408/429/5xx 自动重试 3 次（300ms / 600ms 退避）。
- 缓存仍只在**成功**时写入；没拿到就保持"未拉取"状态，下一次拦截 `/v1/models` 会再试一次。

v0.1.19 让规格覆盖**同名但不是本插件注册**的模型：

- 拦截器除了补自己注册的模型，还会按**名字最后一段**在厂商目录里查：例如同一个模型通过
  CPA 的内置 OpenAI 兼容路由接入（名字相同、但那条路由不会带参数），也能被补上。
- 目录结果会**缓存在插件里**（刷新时填充；没有缓存时在后台补拉一次，不在响应路径上等网络），
  所以拦截 `/v1/models` 不会引入额外延迟。
